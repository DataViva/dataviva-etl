...
SELECT year, bra_id as Brasil, course_sc_id as Curso,
        ...
        CASE WHEN dataviva.sc_ybc.bra_id_len = 9 THEN null ELSE(select bra_id from dataviva.sc_ybc a JOIN dataviva.attrs_bra b ON a.bra_id = b.id where year = '2014' and bra_id_len = '9' and course_sc_id = dataviva.sc_ybc.course_sc_id and bra_id like concat(dataviva.sc_ybc.bra_id, '%') order by enrolled desc limit 1) END as Top_Municípios_Matrículas,
        CASE WHEN dataviva.sc_ybc.bra_id_len = 9 THEN null ELSE(select enrolled from dataviva.sc_ybc a JOIN dataviva.attrs_bra b ON a.bra_id = b.id where year = '2014' and bra_id_len = '9' and course_sc_id = dataviva.sc_ybc.course_sc_id and bra_id like concat(dataviva.sc_ybc.bra_id, '%') order by enrolled desc limit 1) END as Matrículas_Municicípio
