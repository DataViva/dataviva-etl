...
"""
    ...
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    How to run: python -m exportacao.datasent.step_1_hs -m all
    If need to run just a specific check, run: python -m exportacao.datasent.step_1_hs -m HS
    
    Running one by one:
    
    python -m exportacao.datasent.step_1_hs -m HS > HS.log
    python -m exportacao.datasent.step_1_hs -m Municipality > Municipality.log 
    python -m exportacao.datasent.step_1_hs -m State > State.log 
    python -m exportacao.datasent.step_1_hs -m WLD > WLD.log     
...
def checkHS(year):
    ...
    for hs in dfDV['id']:
        ...
        valDV=to_int(valDV)
        if valDV!=valCSV:
            txt= "ERROR in HS: "+str(hsint)+" / "+str(hs)+" - Value in CSV "+ str(valCSV)+ " <> Value in DV "+str(valDV) + " - Difference: "+str(valCSV - valDV)
            #print txt
        else:
            txt="OK"
            #print txt
...
def checkMunicipality(year,size,column):
    print "Entering in checkMunicipality" 
    ...
    for mdic in dfDV['id']:        
        ...
        try: 
            valCSV= sum(dfGroup.get_group(mdicid)[dfSent.columns[9]])
        except:
...
def checkWLD(year):
    ...
    dfDV = sql_to_df("SELECT a.id_mdic as id,sum(val_usd) as val FROM secex_yb s,attrs_bra a where length(a.id)=8 and  a.id=s.bra_id and year="+str(year)+" group by 1;",db)

    dfSent = read_from_csv("dados\exportacao\sent\MDIC_"+str(year)+".csv",delimiter="|")
    dfGroup = dfSent.groupby(dfSent.columns[5])
