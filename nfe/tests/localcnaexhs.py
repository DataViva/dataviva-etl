...
run=''
#C:\Python27-64\python -m nfe.tests.localcnaexhs
def main(run):
...
def groupBYMunHS(dados):
    ...
    return dados.groupby(['Municipality_ID_Sender','TransactedProduct_ID_HS','EconomicAtivity_ID_CNAE_Sender']).agg({"empresas": np.sum, "Product_Value": np.sum, "corte": np.max, })
