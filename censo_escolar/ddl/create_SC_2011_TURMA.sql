...
drop table if exists SC_2011_TURMA_BLOCO;
create table SC_2011_TURMA_BLOCO(dados varchar(2000) not null);

load data local infile 'Y:/Censo Escolar/2011/DADOS/TS_TURMA.txt'
into table SC_2011_TURMA_BLOCO
character set 'latin1'
fields terminated by '\t'
lines terminated by '\n';

drop table if exists SC_2011_TURMA;
create table SC_2011_TURMA select 
...
substring(dados,170,1) as ID_RECURSOS_BAIXA_VISAO,
substring(dados,171,1) as ID_PROCESSOS_MENTAIS,
substring(dados,172,1) as ID_ORIENTACAO_MOBILIDADE,
substring(dados,173,1) as ID_SINAIS,
substring(dados,174,1) as ID_COM_ALT_AUMENT,
substring(dados,175,1) as ID_ENRIQ_CURRICULAR,
substring(dados,176,1) as ID_SOROBAN,
substring(dados,177,1) as ID_INF_ACESSIVEL,
substring(dados,178,1) as ID_PORT_ESC,
substring(dados,179,1) as ID_AUT_ESCOLAR,
...
substring(dados,226,8) as FK_COD_DISTRITO,
substring(dados,234,1) as ID_LOCALIZACAO,
substring(dados,235,1) as ID_DEPENDENCIA_ADM,
substring(dados,236,1) as DESC_CATEGORIA_ESCOLA_PRIVADA,
substring(dados,237,1) as ID_CONVENIADA_PP,
substring(dados,238,2) as ID_TIPO_CONVENIO_PODER_PUBLICO,
substring(dados,240,1) as ID_MANT_ESCOLA_PRIVADA_EMP,
substring(dados,241,1) as ID_MANT_ESCOLA_PRIVADA_ONG,
substring(dados,242,1) as ID_MANT_ESCOLA_PRIVADA_SIND,
substring(dados,243,1) as ID_MANT_ESCOLA_PRIVADA_S_FINS,
substring(dados,244,1) as ID_DOCUMENTO_REGULAMENTACAO,
substring(dados,245,1) as ID_LOCALIZACAO_DIFERENCIADA,
substring(dados,246,1) as ID_EDUCACAO_INDIGENA
from SC_2011_TURMA_BLOCO;

select * from SC_2011_TURMA;

drop table SC_2011_TURMA_BLOCO;
