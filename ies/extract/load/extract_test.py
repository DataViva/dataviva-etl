import os, sys, click, magic, codecs, time, itertools
...
USAGE EXAMPLE:
python ies/extract/load/extract_test.py ies/extract/data/IES_2009/ALUNO.txt
...
def main(file_path):
    ...
    with codecs.open(file_path, mode='r', encoding=encoding) as fp:
        while True:
            for line in itertools.islice(fp, 300000):
                if not line:
                    break
            row = (
                line[0:8],
